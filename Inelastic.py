...
class Inelastic_kernel():
    ...
    def split_rates(self):
        ...
        # g <-> qq_bar
        c0 = self.k.alphas / (4 * np.pi) * np.sqrt(self.k.qhat)
        c  = np.sqrt((self.k.CF/self.k.Nc) - (self.x - self.x**2)) \
        * (1 - 2*self.x + 2*self.x**2) / np.sqrt(self.x - self.x**2)
        Ig_qq = c * c0

        # q <-> gq
        b0 = (self.k.alphas * self.k.CF) / (2 * np.pi) \
             * np.sqrt(self.k.qhat)
        b  = np.sqrt((1-self.x) + (self.k.CF/self.k.Nc) * self.x**2) \
             * (2 - 2*self.x + self.x**2) / (self.x * np.sqrt(self.x - self.x**2))
        Iq_gq = b * b0

        # q <-> qg (change x <-> (1-x))
        b0 = (self.k.alphas * self.k.CF) / (2 * np.pi) \
             * np.sqrt(self.k.qhat)
        b  = np.sqrt((self.x) + (self.k.CF/self.k.Nc) * (1-self.x)**2) \
